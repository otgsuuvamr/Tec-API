...
exports.create = async (req, res) => {
  ...
  // Tratativas de campos obrigatórios;
  if (!titulo) {
    return res.status(400).json({ error: "Preencha o título." });
  }
  if (!preco) {
    return res.status(400).json({ error: "Preencha o preco." });
  }
  if (!descricao) {
    return res.status(400).json({ error: "Preencha a descricao." });
  }
  if (emEstoque === undefined) {
    return res.status(400).json({ error: "Preencha o status." });
  }
  try {
    // Criação dos produtos;
    const newProd = new Tec({ titulo, preco, descricao, emEstoque });
...
exports.update = async (req, res) => {
  ...
  // Tratativas de campos obrigatórios;
  if (!titulo) {
    return res.status(400).json({ error: "Preencha o novo título." });
  }
  if (!preco) {
    return res.status(400).json({ error: "Preencha o novo preco." });
  }
  if (!descricao) {
    return res.status(400).json({ error: "Preencha a nova descricao." });
  }
  if (!emEstoque) {
    return res.status(400).json({ error: "Preencha o novo status." });
  }
  try {
...
exports.readID = async (req, res) => {
  ...
  try {
    ...
    if (!product) {
      return res.statsu(404).json({ error: "Produto não encontrado." });
    }
    res.status(200).json({
    ...
    return res.statsu(400).json({ error: "Erro ao ler produto." });
  }
};
